...
- **Rapport d'aide à la décision (advisory tier, display/report-only)** : `reports/` (scoring pur + portfolio_advisor + top1000 CoinGecko + generator JSON/Markdown + store fichiers) + `workers/report_worker.py` (génération minuit) → rapport quotidien décisionnel sur les ~300 cryptos + top 1000 externe (`/api/reports/daily/*`, `/api/reports/portfolio/model`, `/api/reports/opportunities/top1000`, modale 📅 Report à 7 onglets). Real-data-only ; ne nourrit ni le bot ni la persistance.

Schéma complet : [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#2-schéma-des-flux-de-données).

### Rôle des dossiers (résumé)
`collectors/` (WS par exchange) · `models/` (canonical + event_uid) · `db/` (writer batch+DLQ, migrations) · `workers/` (ingestor, aggregator, feature_worker, social_ingestor, antigravity_bot, outcome_evaluator, report_worker, bootstrap, process_supervisor) · `signal_engine/` (market_features, social_engine, risk_engine, scorer) · `paper_execution/` (engine) · `market/` (hubs binance_spot, universe, global_context, defi) · `reports/` (scoring pur, portfolio_advisor, top1000 CoinGecko, generator JSON/Markdown, store fichiers+index DB — daily report décisionnel) · `social/` (base, analyzer, mock, rss) · `api/` (FastAPI + WS + hubs + cockpit statique) · `frontend/` (cockpit) · `scripts/` (supervisor + .ps1) · `tests/` (offline) · `docs/`.
...
### Rapport quotidien v2 — aide à la décision financière (PR9)
**But** : transformer le rapport « indicatif/pédagogique » (PR8) en **rapport d'aide à la décision financière — recommandations générées par modèle quantitatif** (cadrage assumé, sans prétention au conseil personnalisé réglementé ; mention volatilité/dépendance aux données). Détail : [docs/daily_crypto_report.md](docs/daily_crypto_report.md).
- **`reports/scoring.py` (formules spec)** : Opportunity = 25 momentum + 20 trend + 15 volume + 15 force-relative + 10 liquidité + 10 régime **− 5 risk-penalty** (risk calculé d'abord) ; Risk = 30 vol + 25 drawdown + 20 illiquidité + 15 qualité-données + 10 microstructure (slot « concentration » intenable sans données on-chain — real-data-only). **BUY** : opp ≥ 75 **et** risk ≤ 60 **et** conf ≥ 65 **et** momentum > 0.5 **et** liquidité ≥ 0.35 ; **SELL** ajoute la rupture de tendance (prix < VWAP 24h + drawdown ≥ 0.5) ; **AVOID** ajoute la garde **pump suspect** (|24h| ≥ 18 % sans support volume < 0.35). Nouveaux purs : `sub_scores()` (0–100, `None` honnête), `conviction()` (forte/moyenne/faible), `contradictions()`.
- **`reports/portfolio_advisor.py`** (pur) : posture (offensif/équilibré/défensif/cash majoritaire) depuis régime+breadth+F&G ; **allocations modèles 3 profils** (prudent/équilibré/agressif, somme = 100, mid/small → cash si breadth < 0.5) ; tiers par **percentile de volume 24h réel** (proxy mcap assumé, jamais fabriqué) ; actions (acheter/renforcer/conserver/surveiller/alléger/vendre/éviter) ; **poids plafonnés par règles codées** (small illiquide : 0 % prudent / ≤ 1 % sinon ; volatilité ≥ 0.7 : cap ÷ 2 ; max 5/8/12 % par profil).
- **`reports/top1000.py`** : watchlist externe **CoinGecko `/coins/markets`** (4×250, sans clé, 1×/génération, 1.5 s entre pages) — classification pure suivies/nouvelles opportunités/exclues (raison), statuts honnêtes `ok/partial/unavailable/disabled`, lecture partielle conservée, jamais bloquant.
- **`reports/generator.py` (schema v2)** : `executive_summary` (posture, conviction globale, confiance modèle), `portfolio_models`, par actif : `scores` (sous-scores 0–100), `conviction`, `action`, `rationale` décisionnelle (métriques déclencheuses chiffrées + risque principal + contradictions), **niveaux invalidation/TP/SL = niveaux 24h réels uniquement** (low/high/VWAP, raison si absents), `recommended_weights`/`max_weight`, **evidence horodatée** (`as_of_ms`/`age_ms`, raisons FR — fin du « No real source evidence available »), `data_quality`, **`changes_vs_previous`** (diff vs rapport de date strictement antérieure : transitions de signal, baisses de confiance ≥ 15 pts, entrées/sorties).
- **Endpoints ajoutés** : `GET /api/reports/daily/{date}/crypto/{symbol}` (alias `latest`), `GET /api/reports/opportunities/top1000`, `GET /api/reports/portfolio/model`. Worker + `POST /generate` injectent rapport précédent + watchlist externe.
- **Front** : modale 📅 à **7 onglets** (Synthèse/Portefeuille/Opportunités/Risques/Classement/Sources/Historique) — bloc positionnement recommandé, barres de distribution, cartes profils (allocation empilée, drawdown estimé, positions BUY suggérées), cartes BUY avec TP/SL, carte momentum×risque (≤300 dots), **radar SVG** des sous-scores, evidence horodatée, frise du régime. **Zéro `N/A` brut** : `unavail()` rend « Donnée indisponible » + raison au survol.
- **Config** : `ENABLE_TOP1000_WATCHLIST` (défaut True), `TOP1000_PAGES` (4), `TOP1000_MIN_VOLUME_USD` (1M) — réutilise `COINGECKO_API_BASE`/`COINGECKO_API_KEY`/`GLOBAL_CONTEXT_HTTP_TIMEOUT`. Pas de migration (JSON v2 dans les fichiers ; index 008 inchangé). Tests : `test_portfolio_advisor.py`, `test_top1000.py`, extensions `test_daily_report.py`.
