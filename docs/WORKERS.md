...
- **Rôle** : génère le **rapport quotidien d'aide à la décision financière** (advisory tier, modèle quantitatif) sur les ~300 cryptos de l'univers + watchlist externe top 1000. **Display/report-only** — ne touche ni le bot ni la persistance marché. Détail : [daily_crypto_report.md](daily_crypto_report.md).
- **Entrées** : tiers live via l'**API locale** (`GET /api/market/universe`, `/api/market/global` ; `DAILY_REPORT_API_BASE`) ; **CoinGecko `/coins/markets`** (top 1000, best-effort jamais bloquant — `ENABLE_TOP1000_WATCHLIST`, `TOP1000_PAGES`, `TOP1000_MIN_VOLUME_USD`) ; **rapport précédent** sur disque (date strictement antérieure) pour le diff `changes_vs_previous`. Config : `ENABLE_DAILY_REPORT`, `DAILY_REPORT_HOUR/MINUTE/TIMEZONE`, `DAILY_REPORT_UNIVERSE_LIMIT`, `DAILY_REPORT_TOP_N`, `DAILY_REPORT_DIR`, `DAILY_REPORT_PERSIST_DB`.
- **Sorties** : fichiers `reports/daily_crypto_report_YYYY-MM-DD.json|.md` (**source de vérité**, schema v2 : executive_summary, portfolio_models, watchlist_external, data_quality, changes_vs_previous) + miroir DB best-effort (`daily_crypto_report` + `daily_crypto_asset_score`, migration 008 / `reports.store.ensure_schema`). Chiffres calculés par `reports.scoring` (pur) ; portefeuille par `reports.portfolio_advisor` (pur) ; assemblage `reports.generator`.
