...
- **Modales** : Logs, Drill-down (waterfall des facteurs + **Source Evidence** structurée), Timeline, **🖥 Ops / Terminals**, **🏦 DeFi** (top protocoles par TVL — DefiLlama, table rang/nom/catégorie/chaînes/TVL/24h/7j + breakdown par catégorie, **données réelles uniquement** : `n/a`/vide honnête si hub off), **📅 Report** (rapport quotidien **d'aide à la décision financière**, v2 à **7 onglets** : Synthèse — positionnement recommandé/conviction/régime + barres de distribution + top actions ; Portefeuille — 3 cartes profil avec barre d'allocation empilée, drawdown estimé, positions BUY suggérées ; Opportunités — cartes BUY avec rationale + TP/SL/invalidation + watchlist top 1000 CoinGecko ; Risques — SELL/AVOID + carte momentum×risque ≤300 dots ; Classement — table filtrable/triable/recherchable avec action+conviction, détail au clic = **radar SVG** des sous-scores + evidence horodatée ; Sources — qualité de données par source ; Historique — diff vs veille + frise du régime. `setupDailyReport()`/`switchReportTab()`/`renderReport*()` ; cadrage « recommandations générées par modèle quantitatif » affiché ; **zéro `N/A` brut** : `unavail()` rend « Donnée indisponible » + raison au survol), **🔬 Live Source Debug**, Docs.
...
## Endpoints consommés
REST : `/api/binance/config`, `/api/watchlist`, `/api/market/universe?limit=300`, `/api/market/global`, `/api/market/defi?limit=50`, `/api/market/source`, `/api/market/symbol/{symbol}/klines?range=…`, `/api/historical/{symbol}`, `POST /api/market/active-symbol`, `/api/portfolio`, `/api/signals`, `/api/signals/{symbol}`, `/api/decision/{id}`, `/api/market-features/{symbol}`, `/api/trades/recent`, `/api/system/logs`, `/api/binance/debug/{symbol}`, `/api/reports/daily/latest`, `/api/reports/daily/history`, `POST /api/reports/daily/generate`.
